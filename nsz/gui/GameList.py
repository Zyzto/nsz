...
class SelectableRecycleBoxLayout(
    ...
    def keyboard_on_key_down(self, window, keycode, text, modifiers):
        ...
        if (
            keycodeName == "delete"
            or keycodeName == "backspace"
            and len(self.selected_nodes) > 0
        ):
            ...
            self.data.append(
                {
                    "0": path,
                    "1": items[path][0],
                    "2": "v" + str(items[path][1]),
                    "3": self.sizeof_fmt(items[path][2]),
                }
            )
...
class GameList(StackLayout):
    ...
    def handledrops(self, window, rawPath, x, y):
    ...
    def addFiles(self, path):
        ...
        try:
            if path.is_dir():
                ...
                for file in scandir(str(path)):
                    ...
                    if isGame(filepath) or isCompressedGameFile(filepath):
                        ...
                        if isGame(filepath):
                            extractedIdVersion = (
                                FileExistingChecks.ExtractTitleIDAndVersion(filepathStr)
                            )
                ...
                if isGame(path) or isCompressedGameFile(path):
                    if isGame(path):
                        extractedIdVersion = (
                            FileExistingChecks.ExtractTitleIDAndVersion(fullPath)
                        )
                        if extractedIdVersion is None:
                            Print.error(
                                801,
            ...
            Print.error(
                802, "Error while adding {0} to gamelist: {1}".format(fullPath, str(e))
            )
            Print.exception()
